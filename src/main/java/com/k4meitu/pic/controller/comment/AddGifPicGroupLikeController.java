...
public class AddGifPicGroupLikeController {
	...
	public Map<String, Object> resultMap(Map<String, String> param){
		...
		int commentLike = Integer.valueOf(param.get("like"));
		int commentDislike = Integer.valueOf(param.get("dislike"));
		int id = Integer.valueOf(param.get("id"));
		
		
		if (commentLike+commentDislike == 1) {
			try {
				int res = commentService.addGifPicGroupLikeAndDislike(commentLike, commentDislike, id);
				if (res > 0) {
